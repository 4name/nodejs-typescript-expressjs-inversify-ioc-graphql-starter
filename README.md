...
[![Dependency Status](https://david-dm.org/4name/nodejs-typescript-expressjs-inversify-ioc-graphql-starter.svg)](https://david-dm.org/4name/nodejs-typescript-expressjs-inversify-ioc-graphql-starter)
[![Build Status](https://travis-ci.org/4name/nodejs-typescript-expressjs-inversify-ioc-graphql-starter.svg?branch=master)](https://travis-ci.org/4name/nodejs-typescript-expressjs-inversify-ioc-graphql-starter)
...
- Install [Node.js v8](https://nodejs.org/en/) by using nvm
- Install [JetBrains](https://www.jetbrains.com/idea/) (optional)

# Getting started
- Clone the repository
```
git clone --depth=1 https://github.com/Teremok/nodejs-typescript-expressjs-inversify-ioc-graphql-starter.git <project_name>
```
...
| **src/config**           | Project config code. Currently stores Inversify configuration.                                |
...
| **src**/app.ts           | Configured ExpressJs application                                                              |
...
| .graphqlconfig.yaml      | GraphQL configuration                                                                         |
...
| ------------------------- | ------------------------------------------------------------------------------------------------- |
| `start`                   | Runs full build before starting all watch tasks. Can be invoked with `yarn start`                 |
| `debug`                   | Debugs full build before starting all watch tasks.                                                |
...
| `watch:node`              | Continuously monitors `dist/serve.js`                                                             |
| `watch:debug`             | Same as `watch` but continuously watches all files and re-compiles when needed                    |
...
For example, if we want the `.d.ts` file for jQuery, we can do so with `yarn add @types/jquery --dev`.

> **Note!** Be sure to add `--dev` to your `yarn add`. `.d.ts` files are project dependencies, but only used at compile time and thus should be dev dependencies.
...
A majority of this quick start's content was inspired or adapted from:
- Sahat's excellent [Hackathon Starter project](https://github.com/sahat/hackathon-starter).
- [Microsoft Node Starter project](https://github.com/Microsoft/TypeScript-Node-Starter/).
